...
- **User.is_approved**：管理員核准後才能使用 LLM 功能（`get_approved_user` 依賴注入檢查）；`shopee_affiliate_id` / `default_sub_id` 儲存用戶的蝦皮聯盟行銷設定（`PATCH /auth/me` 更新）
...
| `/api/auth/me` | PATCH | 更新用戶個人設定（蝦皮聯盟 ID、Sub ID） |
...
| `/api/articles/generate` | POST | 同步生成文章（等待 LLM 完成後回傳，前端 timeout 3 分鐘） |
...
| `/api/shopee/find-competitors` | GET | 蝦皮競品搜尋（LLM 提取關鍵字 + 分數排名 Top 10，需已核准）|
...
| `/api/keywords/research` | POST | 完整關鍵字研究（種子詞+Autocomplete+LLM策略，需已核准）|
| `/api/keywords/autocomplete` | GET | Autocomplete 預覽（需已核准）|
...
**同步生成**：`POST /generate` 建立 placeholder Article 後，使用 `asyncio.to_thread()` 在執行緒池中同步等待 LLM 生成完成，再回傳完整文章（status=draft/failed）。生成函數使用獨立 DB session（`get_db_session()`），成功更新為 `status="draft"`，失敗更新為 `status="failed"`（content 存詳細錯誤報告含 traceback）。前端 `generateArticle` API 呼叫 timeout 設為 180 秒（3 分鐘）。Cloud Run 部署使用預設 `--cpu-throttling`（無流量時 scale to zero，節省費用）。
...
`llm_service`、`seo_service`、`keyword_research_service` 在模組層級實例化為單例，直接 import 使用。

### SEO 長尾關鍵字研究

Phase 1 不建 DB model，研究結果僅存前端 state，生成文章時透過 `keyword_strategy` 欄位傳入後端。
流程：前端選商品 → `POST /api/keywords/research`（種子詞提取 → A-Z Autocomplete 展開 → LLM 策略 JSON）→ 前端顯示策略 → 使用者確認後帶入 `generateArticle(payload)` → 後端 `format_keyword_context()` 注入 user_message。
強制使用 `gemini-2.5-flash`，每次研究約 NT$0.03。Autocomplete 請求間隔 0.5 秒避免被封。
...
- [x] 商品探索儲存連結（📌存連結到 localStorage + 商品管理頁勾選複製面板，最多 5 個）
- [x] 文章發佈狀態管理（貼到 Dcard 自動標記已發佈 + 手動狀態切換下拉選單）
- [x] 系統提示詞開關（設定頁可停用 SYSTEM_INSTRUCTIONS / SEO 優化提示詞）
- [x] 商品探索自動填滿（結果不足 20 筆自動分頁，最多 10 頁）
- [x] 商品探索關鍵字相關性過濾（搜尋結果僅保留包含關鍵字的商品）
- [x] 競品搜尋精準度優化（智能 fallback + emoji 過濾 + 片段補全 + 確保 3 關鍵字）
- [x] SYSTEM_INSTRUCTIONS SEO 最佳化規則（關鍵字策略、FAQ 結構、內容結構、可讀性）
- [x] SEO 長尾關鍵字研究 Phase 1（Autocomplete 展開 + LLM 策略生成 + 注入文章生成 + 前端面板）
- [x] 用戶專屬蝦皮聯盟導購連結（設定頁填入 Affiliate ID → 存連結自動組裝專屬追蹤連結）
- [ ] 批量生成
...
**⚠️ BLOCKING：所有部署操作（Cloud Run / Firebase Hosting）執行前必須先詢問用戶確認。**
禁止自動部署。每次要跑 `gcloud builds submit`、`gcloud run deploy`、`firebase deploy` 之前，必須先告知用戶本次變更摘要並取得明確同意。Cloud Run build 費用高，避免不必要的重複部署。
